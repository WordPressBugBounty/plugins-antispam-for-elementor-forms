...
class Elementor {
	...
	public function validate_submission( Form_Record $form, Ajax_Handler $ajax ) {
		if( $this->is_enabled() ) {
			...
			$excluded_field_types = (array) apply_filters( 'asef/excluded_field_types', [
				'radio',
				'select',
				'checkbox',
				'acceptance',
				...
				'step',
				'asef-js-honeypot',
			] ); // Exclude fields without user-inputted content
			...
			foreach( $words as $word ) {
				...
				foreach( $form->get( 'fields' ) as $field ) {
					if( !in_array( $field['type'], $excluded_field_types ) && is_scalar( $field['value'] ) && preg_match( $pattern, $field['value'] ) ) {
	...
	public function get_block_list(): array {
		if( null === $this->block_list ) {
			...
			// Remove allow list items from the remote and custom lists
			foreach( $allow_list as $allow_list_word ) {
				...
				foreach( $custom_list as $custom_list_word_key => $custom_list_word ) {
					if( preg_match( $pattern, $custom_list_word ) ) {
						unset( $custom_list[$custom_list_word_key] );
					}
				}
			}
